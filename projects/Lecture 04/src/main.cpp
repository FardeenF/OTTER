...
int main() {
	...
	static const GLfloat points[] = {
		-0.5f, -0.5f, 0.0f,
		0.5f, -0.5f, 0.0f,
		0.0f, 0.5f, 0.0f // Rotate around this point
	};
	...
	//Projection Matrix - 45 degrees FoV(Field of View), ratio, range 0.1 - 100 units
	int width, height;
	glfwGetWindowSize(window, &width, &height);

	glm::mat4 Projection = glm::perspective(glm::radians(45.0f), (float)width / (float)height, 0.1f, 100.0f);

	//Camera						(camera position, camera target, camera orientation)
	glm::mat4 View = glm::lookAt(glm::vec3(0, 0, 3), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));

	//Model Matrix
	glm::mat4 Model = glm::mat4(1.0f); //Identity Matrix - Resetting the matrix
	...
	//Handle for our mvp matrix (uniform variable - contents will not change for an entire draw call)
