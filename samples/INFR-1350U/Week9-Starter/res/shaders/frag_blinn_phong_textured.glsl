...
uniform float u_ReflectLightStrength;
...
void main() {
	...
	//vec3 convertedColor = vec3(environment);

	float texRef = texture(s_Reflectivity, inUV).x;
	float ref = pow(max(dot(N, h), 0.0), u_Shininess);
	vec3 reflector = u_ReflectLightStrength * texRef * ref * u_LightCol;

	vec3 reflectionColor = mix(textureColor.rgb, environment, texRef);

	vec3 result = (
		(u_AmbientCol * u_AmbientStrength) + // global ambient light
		(ambient + diffuse + specular) * attenuation // light factors from our single light
		) * textureColor.rgb + reflectionColor; // Object color
