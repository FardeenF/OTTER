...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->SetUniform("u_ReflectLightStrength", lightReflectPow);
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Light Level Lighting Settings"))
			{
				...
				if (ImGui::SliderFloat("Light Reflect Power", &lightReflectPow, 0.0f, 1.0f)) {
					shader->SetUniform("u_ReflectLightStrength", lightReflectPow);
				}
		...
		Texture2D::sptr diffuse = Texture2D::LoadFromFile("images/box.bmp");
		...
		material0->Set("u_TextureMix", 0.8f);
		...
		reflectiveShader->LoadShaderPartFromFile("shaders/frag_blinn_phong_textured.glsl", GL_FRAGMENT_SHADER);
