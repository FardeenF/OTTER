...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "Terrain Assignment", nullptr, nullptr);
...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		GameObject spriteObj = scene->CreateEntity("HeartSprite");
		{
			...
			spriteObj.emplace<Sprite>().SetMesh(spriteVao).SetMaterial(spriteMaterial);
			spriteObj.get<Transform>().SetLocalPosition(0.0f, 0.0f, 8.0f);
			spriteObj.get<Transform>().SetLocalRotation(0.0f, 180.0f, 0.0f);
		...
		GameObject obj5 = scene->CreateEntity("Terrain Plane");
		{
			...
			obj5.get<Transform>().SetLocalPosition(-40.0f, 0.0f, -15.1f).SetLocalScale(glm::vec3(20.f));
