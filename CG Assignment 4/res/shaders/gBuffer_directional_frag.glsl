...
void main() 
{
	...
	vec3 pPosition = texture(s_positionTex, inUV).xyz;
	vec3 pColor = texture(s_specularTex, inUV).xyz;
	vec3 pNormal = texture(s_normalsTex, inUV).xyz;
	vec3 pViewDir = u_CamPos - pPosition;
	...
	float pointDistance = length(pLight.position.xyz - fragPos);

	if(pointDistance < pLight.radius)
	{
		result += CalcPointLight(pLight, pNormal, fragPos, pViewDir);
	}
	...
	//frag_color = vec4(CalcPointLight(pLight, pNormal, fragPos, pViewDir), 1.0);
}
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
	...
	// attenuation
	float pDistance    = length(light.position.xyz - fragPos);
	float attenuation = 1.0 / (light.constant + light.linear * pDistance + 
  					light.quadratic * (pDistance * pDistance));    
	// combine results
	vec3 ambient  = light.ambient.xyz  * vec3(texture(s_albedoTex, inUV));
	vec3 diffuse  = light.diffuse.xyz  * diff * vec3(texture(s_albedoTex, inUV));
	vec3 specular = light.specular.xyz * spec * vec3(texture(s_albedoTex, inUV));
	ambient  *= attenuation;
	diffuse  *= attenuation;
