...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		//Init lights shader
		Shader::sptr lightsShader = Shader::Create();
		lightsShader->LoadShaderPartFromFile("shaders/MultLights_vert.glsl", GL_VERTEX_SHADER);
		lightsShader->LoadShaderPartFromFile("shaders/MultLights_frag.glsl", GL_FRAGMENT_SHADER);
		lightsShader->Link();
		
		Shader::sptr cubeShader = Shader::Create();
		cubeShader->LoadShaderPartFromFile("shaders/LightCube_vert.glsl", GL_VERTEX_SHADER);
		cubeShader->LoadShaderPartFromFile("shaders/LightCube_frag.glsl", GL_FRAGMENT_SHADER);
		cubeShader->Link();
		...
		glm::vec3 lightMeshPos = glm::vec3(2.0f, 0.0f, 4.0f);
		float lightMeshScale = 0.3f;
		
		float difference = 0.198f;
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Light Controller"))
			{
				
				if (ImGui::DragFloat3("Light Position", glm::value_ptr(lightMeshPos), 0.0f, -10.0f, 10.0f))
				{

				}
				if (ImGui::SliderFloat("Light Scale", &lightMeshScale, 0.1f, 10.0f))
				{
			...
			if (ImGui::CollapsingHeader("Point Light"))
			{
				...
				if (ImGui::DragFloat3("Light Position", glm::value_ptr(illuminationBuffer->GetPointLightRef().position), 0.1f, -100.0f, 100.0f))
				{

				}
				if (ImGui::DragFloat3("Ambient", glm::value_ptr(illuminationBuffer->GetPointLightRef().ambient), 0.01f, 0.0, 10.0))
				{

				}
				if (ImGui::DragFloat3("Diffuse", glm::value_ptr(illuminationBuffer->GetPointLightRef().diffuse), 0.01f, 0.0, 10.0))
				{

				}
				if (ImGui::DragFloat3("Specular", glm::value_ptr(illuminationBuffer->GetPointLightRef().specular), 0.01f, 0.0, 10.0))
				{
					
				}
				if (ImGui::SliderFloat("Constant Value", &illuminationBuffer->GetPointLightRef().constant, 0.1f, 10.0f))
				{

				}
				if (ImGui::SliderFloat("Linear Value", &illuminationBuffer->GetPointLightRef().linear, 0.1f, 10.0f))
				{

				}
				if (ImGui::SliderFloat("Quadratic Value", &illuminationBuffer->GetPointLightRef().quadratic, 0.1f, 10.0f))
				{

				}
				if (ImGui::SliderFloat("Radius", &illuminationBuffer->GetPointLightRef().radius, 0.1f, 30.0f))
				{

				}
			}
			if (ImGui::SliderFloat("Difference", &difference, 0.0f, 5.0f))
			{
		...
		ShaderMaterial::sptr BlankMat = ShaderMaterial::Create();
		BlankMat->Shader = gBufferShader;
		BlankMat->Set("s_Diffuse", cloud);
		BlankMat->Set("s_Specular", stoneSpec);
		BlankMat->Set("u_Shininess", 8.0f);
		BlankMat->Set("u_TextureMix", 0.0f);


		VertexArrayObject::sptr lightMeshVAO = ObjLoader::LoadFromFile("models/LightMesh.obj");

		GameObject LightMesh = scene->CreateEntity("LightMesh");
		{
			LightMesh.emplace<RendererComponent>().SetMesh(lightMeshVAO).SetMaterial(BlankMat).SetCastShadow(false);
			LightMesh.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			LightMesh.get<Transform>().SetLocalPosition(lightMeshPos);
			LightMesh.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
		}
		...
		VertexArrayObject::sptr sphereVao;
		GameObject sphereMesh = scene->CreateEntity("Sphere");
		{
			MeshBuilder<VertexPosNormTexCol> sphere;
			MeshFactory::AddIcoSphere(sphere, glm::vec3(0.0f), 1.0f);
			//MeshFactory::InvertFaces(sphere);
			sphereVao = sphere.Bake();
			sphereMesh.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			sphereMesh.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
			sphereMesh.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
		}
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			//Move Light Mesh
			//LightMesh.get<Transform>().SetLocalPosition(lightMeshPos);
			//LightMesh.get<Transform>().SetLocalScale(lightMeshScale, lightMeshScale, lightMeshScale);
			...
			LightMesh.get<Transform>().SetLocalPosition(glm::vec3(illuminationBuffer->GetPointLightRef().position));
			//LightMesh.get<Transform>().SetLocalScale(glm::vec3(illuminationBuffer->GetPointLightRef().radius - difference));
			LightMesh.get<Transform>().SetLocalScale(glm::vec3(illuminationBuffer->GetPointLightRef().ambient.x - difference));
			...
			glClearColor(0.0f, 0.5f, 0.7f, 0.3f);
			glEnable(GL_DEPTH_TEST);
			glClearDepth(1.0f);
			...
			//Set Illumination Stuff
			illuminationBuffer->SetShader(gBufferShader);
			illuminationBuffer->SetTransform(LightMesh.get<Transform>());
			illuminationBuffer->SetViewProjection(viewProjection);
			illuminationBuffer->SetVAO(lightMeshVAO);
			...
			//BackendHandler::RenderVAO(gBufferShader, sphereVao, viewProjection, sphereMesh.get<Transform>(), lightSpaceViewProj);
			...
			illuminationBuffer->ApplyPointEffect(gBuffer);
